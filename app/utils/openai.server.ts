...
const SYSTEM_PROMPT = `Attached is a slide from a lecture. Explain the content of the slide to a student learning the material.
...
export function buildSummaryQuery({
  ...
  const assistantMessages: OpenAI.Chat.ChatCompletionMessageParam[] =
    contextSlides
      ...
      : [];
  const slideQuery = buildSlideQuery(base64Encoding);
